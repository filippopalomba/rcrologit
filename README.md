...
An alternative, handier way to rewrite the model above is to define $Z_{i\ell}:=\sum_{j=1}^J$ as the interaction between $Z_i$ and a dummy for choice $\ell$ and similarly for $V_{i\ell}$, and consider the equivalent model
